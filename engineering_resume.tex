...
\resitem{Designed microbial experiments with complex fluids and materials; applied stochastic, random-walk methods (spatial stochastic differential equations) to model the evolutionary dynamics of observed colony growth}
...
\begin{myitemize}
\skillitem{\textbf{\textit{Summary}:} Expert ability to create experiments, models, and numerical simulations to study the transport of mass, momentum, and energy in complex fluids and materials}
\skillheader{Analytical and Numerical Skills}
\begin{myitemize}
\skillitem{Ability to mesh and simulate fluid flows containing chemical reactions using standard open-source tools (OpenFOAM, gmsh, SALOME) or custom-built GPU-powered Lattice Boltzmann tools}
\skillitem{Experience simulating multicomponent multiphase flows using the Lattice Boltzmann technique}
\skillitem{Expert knowledge of Applied Mathematics, including stochastic modeling involving the Master equation, the Fokker-Planck equation (PDEs), and (spatial) stochastic differential equations}
\skillitem{Ability to use core physics training to create and calibrate models to match experiments }
\end{myitemize}


\skillheader{Experimental Skills}
\begin{myitemize}
\resitem{Adept at designing and conducting biological and soft matter experiments involving complex fluids; four years of research in a molecular biology laboratory}
\resitem{Expertise in use of rheometers to quantify fluid rheology }
\resitem{Significant experience using microscopy to image microbial colonies and analyzing images via computational tools}
\end{myitemize}
